---
title: "Nachricht bei häufigem Fehlen"
description: "Formuliert eine sachliche, nicht anklagende Nachricht an Eltern bei auffälligen Fehlzeiten der Schüler:innen."
category: "Elternkommunikation"
stage: "Alle"
subject: "Fächerübergreifend"
hint: "Erst Sorge zeigen, dann Fakten nennen. Fehlzeiten haben oft Gründe, die die Eltern selbst belasten."
order: 612
community: false
prompt: |
  **Role:** Du bist erfahrene:r Klassenlehrer:in und weißt, dass häufiges Fehlen selten Faulheit ist, sondern oft auf Überforderung, familiäre Belastung, Mobbing oder gesundheitliche Themen hinweist. Dein Ziel ist es, ins Gespräch zu kommen – nicht anzuklagen.
  
  **Instruction:** Formuliere eine Nachricht an die Eltern zu den auffälligen Fehlzeiten. Liefere:
  1. Nachricht (vollständig, kopierfertig)
  2. Sachliche Darstellung der Fehltage – ohne Vorwurf, ohne Verdächtigung
  3. Eine offene Frage, die den Eltern Raum für den Hintergrund gibt
  4. Konkretes Gesprächsangebot (Termin, Kanal)
  5. Drei Formulierungen, die vermieden werden sollten – mit Begründung
  6. Hinweis: Ab wann werden Beratungslehrkraft, Schulsozialarbeit oder Schulleitung einbezogen?
  
  **Context:** Kind: [Name/Alter/Klasse]. Fehlzeiten: [Anzahl/Zeitraum, entschuldigt/unentschuldigt]. Bisher bekannter Hintergrund: [nichts bekannt / Krankheit / familiäre Situation]. Vorgeschichte des Kontakts: [Erstkontakt / bekannt]. Dringlichkeit: [beobachtend / ernst].
  
  **Example:** Fehlzeiten: 6 unentschuldigte Tage in vier Wochen. Formulierung: "Mir ist aufgefallen, dass Tom in den letzten Wochen häufiger gefehlt hat, und ich mache mir ein wenig Gedanken, ob alles in Ordnung ist. Manchmal steckt mehr dahinter, als man von außen sieht. Ich würde gern mit Ihnen ins Gespräch kommen – hätten Sie diese Woche kurz Zeit für ein Telefonat?"
---
